...
public class MysqlOrderDAO implements OrderDAO {
	...
	public int makeOrder(Integer userId, Map<Product, Integer> items, Delivery delivery) 
		...
		try {
			...
			for (Entry<Product, Integer> e : items.entrySet()) {
				log.debug("Try add telephone to the order." + e);
				addtelephoneHasOrder(con, e.getKey().getId(), orderId, e.getValue());
			}
			con.commit();
		} catch (SQLException e) {
	...
	private void addtelephoneHasOrder(Connection con, Integer telephoneId, int orderId, Integer count) 
			throws SQLException {
		log.trace("Start");
		
		PreparedStatement st = null;
		try {
			st = con.prepareStatement(Querys.SQL_INSERT_TELEPHONE_HAS_ORDER);
			st.setInt(1, telephoneId);
			st.setInt(2, orderId);
			st.setInt(3, count);
			log.debug("Query --> " + st);
			st.executeUpdate();
	...
	private List<Order> listOrders(Connection con, String pattern, String orderColumn,
		...
		try {
			...
			String whereTelephones = " WHERE `order_id` IN " + SqlUtil.listToIN(list);
			query = Querys.SQL_GET_FULL_ORDERS + whereTelephones + order;
			st = con.prepareStatement(query);
			log.debug("Query --> " + query);
			rs = st.executeQuery();
			while (rs.next()) {
	...
	Product unmapProductForOrder(ResultSet rs) throws SQLException {
		Product item = new Telephone();
		item.setId(rs.getInt("telephone_id"));
